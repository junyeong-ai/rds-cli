...
download_binary() {
    local version="$1"
    local target="$2"
    local archive="rds-cli-v${version}-${target}.tar.gz"
    ...
    if check_skill_exists; then
        ...
        case "$choice" in
            ...
            1|"")
                ...
                echo "  • Explore database schemas automatically"
                echo "  • Execute queries with natural language"
                echo "  • Manage saved queries"
                echo "  • Format results as JSON/CSV/Table"
                echo "  • Ensure query safety with validation"
                ;;
            *)
                echo ""
